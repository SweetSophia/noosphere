...
const articleContent = nonEmpty
  ...
  .refine(
    ...
    "Content must not exceed 1048576 UTF-8 bytes.",
  );
...
export function createNoosphereMcpServer(
  api: NoosphereApi,
  version: string,
): McpServer {
  const server = new McpServer(
    { name: "noosphere", version },
    {
      instructions:
        "Use search_articles or recall_memory before creating duplicate knowledge. save_memory always creates a draft candidate; create_article is an explicit curated write.",
    },
  );
  ...
  server.registerTool(
    ...
    async (input) => execute(() => api.getArticle(input as GetArticleInput)),
  );
...
async function execute(
  operation: () => Promise<NoosphereResponse>,
): Promise<{
  content: Array<{ type: "text"; text: string }>;
  structuredContent?: NoosphereResponse;
  isError?: boolean;
}> {
  try {
