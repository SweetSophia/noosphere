...
function inspectServer(runCodex: RunCodex): CodexServerRecord | undefined {
  ...
  return parsed as CodexServerRecord;
}
...
function ensureCodexEnvForwarding(
  ...
  const sectionStart = lines.findIndex(
    (line) => line.trim() === `[mcp_servers.${SERVER_NAME}]`,
  );
  if (sectionStart < 0) {
    throw new CodexInstallError(
      "Codex config does not contain the expected [mcp_servers.noosphere] section.",
    );
  }
  ...
  if (!FORWARDED_ENV_VARS.every((name) => readback.includes(`"${name}"`))) {
    throw new CodexInstallError(
      "Codex environment-forwarding readback did not contain every required variable name.",
    );
  }
  return true;
}
...
function restoreServer(
  ...
  if (restored.code !== 0) {
    ...
    if (current && samePlainManagedServer(current, previous)) return;
    throw new CodexInstallError(
...
function samePlainManagedServer(
...
function knownEnvVarList(value: unknown): boolean {
  return value === undefined ||
    (isStringArray(value) &&
      value.every((name) => FORWARDED_ENV_VARS.includes(
        name as (typeof FORWARDED_ENV_VARS)[number],
      )));
}

function notNullOrMissing(value: unknown): boolean {
