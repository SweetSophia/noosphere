...
export async function installCodexIntegration(options) {
    ...
    try {
        ...
        if ((mutatedServer || mutatedCodexConfig) && serverRollbackSucceeded) {
            try {
                assertSafeDirectoryPath(codexHomeDir, "Codex home");
                restoreCodexConfig(codexConfigFile, originalCodexConfig, originalCodexConfigMode, configMutationReceipt);
            }
            catch (rollbackError) {
                rollbackErrors.push(rollbackError instanceof Error ? rollbackError.message : String(rollbackError));
            }
        ...
        if (wroteSkill && existsSync(skillFile)) {
            try {
                ...
                if (!writtenSkillIdentity || !sameFileIdentity(currentIdentity, writtenSkillIdentity)) {
                    throw new CodexInstallError(`Refusing to remove ${skillFile} during rollback because its file identity changed after installation.`);
                }
                if (readFileSync(skillFile, "utf8") === sourceSkill) {
        ...
        for (const created of [...createdSkillDirectories].reverse()) {
            ...
            try {
                assertSafeSkillDirectoryPath(options.homeDir);
                const current = readDirectoryIdentity(created.path, "Created Codex skill directory");
                if (current.dev !== created.dev || current.ino !== created.ino) {
                    throw new CodexInstallError(`Refusing to remove ${created.path} during rollback because its directory identity changed after installation.`);
                }
...
function isExactServer(server, command, args, requireCompleteForwarding = false) {
...
function ensureCodexEnvForwarding(configFile, recordWrite) {
    ...
    if (!stringArrayEquals(readbackEnvVars, mergedEnvVars)) {
        throw new CodexInstallError("Codex noosphere environment-forwarding readback did not exactly match the intended variable names.");
    }
    return true;
}
export function readNoosphereEnvVarsFromConfig(config) {
    const lines = config.replace(/\r\n/g, "\n").split("\n");
    const { sectionStart, sectionEnd } = findNoosphereSectionBounds(lines);
    const assignments = lines.slice(sectionStart + 1, sectionEnd).filter((line) => /^\s*env_vars\s*=/.test(line));
    if (assignments.length !== 1) {
        throw new CodexInstallError("Codex noosphere environment-forwarding readback must contain exactly one env_vars assignment.");
    }
    return parseEnvVarAssignment(assignments[0]);
}
function findNoosphereSectionBounds(lines) {
    const sectionStarts = lines
        .map((line, index) => line.trim() === `[mcp_servers.${SERVER_NAME}]` ? index : -1)
        .filter((index) => index >= 0);
    if (sectionStarts.length !== 1) {
        throw new CodexInstallError("Codex config must contain exactly one unambiguous [mcp_servers.noosphere] section.");
    }
...
function parseEnvVarAssignment(line) {
    ...
    if (new Set(parsed).size !== parsed.length) {
        throw new CodexInstallError("Codex noosphere env_vars contains duplicate environment variable names.");
    }
    return parsed;
}
function restoreCodexConfig(configFile, original, originalMode, receipt) {
    ...
    if (existsSync(configFile) && readFileSync(configFile, "utf8") === original) {
        if (originalMode !== undefined && receipt) {
            const currentIdentity = readRegularFileIdentity(configFile, "Codex config rollback target");
            if (sameFileIdentity(currentIdentity, receipt.identity) &&
...
function assertOwnedConfigMutation(configFile, receipt) {
    if (!receipt) {
        throw new CodexInstallError(`Refusing to restore ${configFile} because the installer has no ownership receipt for its current contents.`);
    }
    const currentIdentity = readRegularFileIdentity(configFile, "Codex config rollback target");
    if (!sameFileIdentity(currentIdentity, receipt.identity) ||
        readFileSync(configFile, "utf8") !== receipt.content) {
        throw new CodexInstallError(`Refusing to restore ${configFile} because its identity or contents changed after the installer wrote it.`);
    }
}
function writeTextAtomically(path, content, defaultMode = 0o600, modeOverride) {
...
function readRegularFileIdentity(path, label) {
...
function readConfigMutationReceipt(configFile) {
    return {
...
function readDirectoryIdentity(path, label) {
    ...
    if (status.isSymbolicLink() || !status.isDirectory()) {
        throw new CodexInstallError(`${label} at ${path} must be a real directory, not a symlink or special file.`);
    }
...
function assertSafeDirectoryPath(path, label) {
    ...
    for (const component of components) {
        ...
        if (status.isSymbolicLink() || !status.isDirectory()) {
            throw new CodexInstallError(`${label} at ${path} must use real directory components, not symlinks or special files.`);
        }
    }
}
...
function ensureSkillDirectories(homeDir) {
    assertSafeDirectoryPath(homeDir, "Home directory");
    const created = [];
    for (const directory of [
        ...
        try {
            mkdirSync(directory);
            created.push(readDirectoryIdentity(directory, "Created Codex skill directory"));
        }
        catch (error) {
...
function restoreServer(runCodex, previous, desiredCommand, desiredArgs) {
    const current = inspectServer(runCodex);
    if (previous && current && samePlainManagedLauncher(current, previous))
        return;
    if (current && !isExactServer(current, desiredCommand, desiredArgs)) {
        throw new CodexInstallError("Refusing to roll back the MCP server because its current launcher is not the installer-owned launcher.");
    }
    if (!previous) {
        if (!current)
            return;
    ...
    if (restored.code !== 0) {
        ...
        if (restoredCurrent && samePlainManagedLauncher(restoredCurrent, previous))
            return;
        throw new CodexInstallError(`Unable to restore the previous MCP launcher: ${boundedCommandError(restored)}`);
    }
}
function samePlainManagedLauncher(current, previous) {
...
export function runCodexCommand(args, codexHomeDir, timeoutMs = CODEX_COMMAND_TIMEOUT_MS) {
...
function knownEnvVarList(value) {
    return isStringArray(value) &&
        ...
        value.every((name) => NOOSPHERE_FORWARDED_ENV_VARS.includes(name));
}
function hasExactForwardedEnvVarSet(value) {
    return isStringArray(value) &&
