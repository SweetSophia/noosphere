...
import { NoosphereArticleCreateRequest } from "../client.js";
...
const ArticleCreateToolParameters = {
  ...
  properties: {
    topicId: {
      type: "string",
      description:
        "The Noosphere topic ID/UUID. Use noosphere_topics first to find the correct ID.",
    },
    title: {
      type: "string",
    ...
    authorName: {
      type: "string",
      description:
        "Optional display author name. Omit to let Noosphere attribute the article from the authenticated context.",
...
export function createNoosphereArticleCreateTool(
  rawConfig: unknown,
  ...
  return {
    ...
    description:
      "Create a curated Noosphere wiki article (not just a draft memory candidate). Use this for structured knowledge worth preserving as a proper wiki article. For simple memory candidates that need editorial review, use noosphere_save instead.",
...
function normalizeArticleCreateParams(rawParams: unknown): {
  ...
  const topicId = readRequiredString(
    params.topicId,
    "topicId",
    ARTICLE_TOPIC_ID_MAX_LENGTH,
  );
  const stripped = stripInjectedMemoryBlocks(
    readRequiredString(params.content, "content"),
  );
  ...
  const authorName = readOptionalString(
    params.authorName,
    "authorName",
...
function readOptionalTags(value: unknown): string[] | undefined {
  ...
  if (value.length > ARTICLE_TAG_MAX_COUNT) throw new Error("too many tags");
  ...
  for (const tag of value) {
    ...
    if (!normalizedSlug) {
      throw new Error(
        "tag must contain at least one ASCII letter or digit because Noosphere tag slugs are ASCII-only",
      );
    }
...
function readOptionalConfidence(
  value: unknown,
): NoosphereArticleCreateRequest["confidence"] {
...
function readOptionalStatus(
  value: unknown,
): NoosphereArticleCreateRequest["status"] | undefined {
  if (value === undefined || value === null || value === "") return undefined;
...
function validateMeaningfulContent(content: string): void {
...
function validateContentByteLength(content: string): void {
...
function stripInjectedMemoryBlocks(content: string): {
  content: string;
  strippedBlocks: string[];
} {
