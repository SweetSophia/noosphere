...
export function createNoosphereCorpusSupplement(context, logger) {
    return {
        async search(input) {
...
function toCorpusSearchResult(result) {
...
function toCorpusGetResult(result, fromLine, lineCount) {
    ...
    const count = normalizePositiveInteger(lineCount, DEFAULT_LINE_COUNT, MAX_LINE_COUNT);
...
function toNoosphereGetRequest(lookup) {
    if (lookup.includes(":")) {
        if (!CANONICAL_REF_PATTERN.test(lookup)) {
            throw new Error("lookup must be a valid canonicalRef or provider-local ID");
        }
        return { canonicalRef: lookup };
    }
    return { provider: "noosphere", id: lookup };
}
function toCorpusPath(result) {
...
function isNoosphereMemoryResult(value) {
...
function warnAndFailOpen(logger, operation, error, context) {
