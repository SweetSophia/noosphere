import { clampTimeout, DEFAULT_AUTO_RECALL_TIMEOUT_MS, isRecord, MAX_AUTO_RECALL_TIMEOUT_MS, readBoolean, readNumber, readString, readStringArray, } from "./config.js";
...
- Use the noosphere_save tool with these parameters:
  - topicId: The actual Noosphere topic ID/UUID where the draft candidate should be filed. Do not guess a slug such as "engineering" unless the runtime explicitly documents slug aliases; inspect existing recall metadata or ask/check the topic list first.
...
export function resolveAutoRecallConfig(rawConfig) {
    ...
    return {
        ...
        timeoutMs: clampTimeout(config.autoRecallTimeoutMs ?? readNumber(process.env.NOOSPHERE_AUTO_RECALL_TIMEOUT_MS), DEFAULT_AUTO_RECALL_TIMEOUT_MS, MAX_AUTO_RECALL_TIMEOUT_MS),
...
export function createNoosphereAutoRecallHook(rawConfig, clientContext, logger) {
     ...
     */
    function ensureCompiledPatterns(patterns, compiled, getLastPatterns, setLastPatterns) {
     ...
     */
    function matchesSessionPatternCached(sessionKey, compiledPatterns) {
    ...
    const hook = async (event, ctx = {}) => {
        ...
        if (sessionKey) {
            ...
            if (effectiveConfig.ignoreSessionPatterns.length > 0) {
                const compiled = ensureCompiledPatterns(effectiveConfig.ignoreSessionPatterns, compiledIgnorePatterns, () => lastIgnorePatterns, (patterns) => { lastIgnorePatterns = patterns; });
            ...
            if (effectiveConfig.skipStatelessSessions && effectiveConfig.statelessSessionPatterns.length > 0) {
                const compiled = ensureCompiledPatterns(effectiveConfig.statelessSessionPatterns, compiledStatelessPatterns, () => lastStatelessPatterns, (patterns) => { lastStatelessPatterns = patterns; });
                if (matchesSessionPatternCached(sessionKey, compiled)) {
        ...
        if (!shouldAutoRecall(effectiveConfig, event, ctx, clientContext.config))
            return;
        const query = buildAutoRecallQuery(event, effectiveConfig);
        if (!query || query.length < effectiveConfig.minQueryLength)
            return;
        try {
 ...
 */
export function buildAutoRecallQuery(event, config) {
    ...
    if (config.includeRecentTurns && Array.isArray(event.messages) && config.recentTurnLimit > 0) {
        ...
        for (const turn of recentTurns) {
            ...
            if (!cleaned || cleaned.length < 5 || seen.has(key))
                continue;
            // Skip if same as the main recall query
            if (cleaned.toLowerCase() === recallQuery.toLowerCase())
                continue;
            seen.add(key);
...
function buildInjectionResult(promptText, position) {
    switch (position) {
        case "system-prepend":
...
function readInjectionPosition(value) {
    if (value === "system-prepend" || value === "system-append")
        return value;
    return "prepend";
}
function extractRecentUserTurns(messages, limit) {
    const turns = [];
...
function shouldAutoRecall(config, event, ctx, resolvedConfig) {
    ...
    if (!hasMessage)
        return false;
    if (config.enabledAgents.length > 0 && !matchesAny(config.enabledAgents, ctx.agentId))
        return false;
    if (config.allowedChatTypes.length > 0 && !matchesAny(config.allowedChatTypes, resolveChatType(ctx)))
        return false;
    return true;
}
function extractPromptInjectionText(response, config) {
    if (response.mode !== "auto")
        return undefined;
    if (typeof response.promptInjectionText !== "string")
        return undefined;
...
function matchesAny(allowed, value) {
    if (!value)
        return false;
