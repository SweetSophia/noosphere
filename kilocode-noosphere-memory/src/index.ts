import type { Plugin, PluginInput, PluginOptions } from "@kilocode/plugin";
...
export const NoosphereKiloPlugin: Plugin = async (
  ctx: PluginInput,
  ...
  return {
    ...
    event: async ({ event }) => {
      ...
      if (!markPendingCapture(sessionId)) return;
    ...
    tool: {
      ...
      noosphere_recall: tool({
        description:
          "Search Noosphere durable memory for project history, prior decisions, runbooks, and technical context.",
        args: {
      ...
      noosphere_save: tool({
        description:
          "Save durable, reusable knowledge to Noosphere as a draft memory candidate. Use only for stable decisions, runbooks, fixes, and project facts.",
        args: {
...
async function shouldInjectRecall(
  ctx: PluginInput,
  sessionId: string,
  injectOn: "first" | "always",
): Promise<boolean> {
  if (injectOn === "always") return true;
