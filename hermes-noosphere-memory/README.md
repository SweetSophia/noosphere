...
## Install the 1.14.0 Release Bundle

The coordinated Noosphere release attaches a deterministic Hermes provider
bundle and checksum to the main GitHub release:

```bash
curl -fsSLO https://github.com/SweetSophia/noosphere/releases/download/v1.14.0/hermes-noosphere-memory-1.14.0.tar.gz
curl -fsSLO https://github.com/SweetSophia/noosphere/releases/download/v1.14.0/hermes-noosphere-memory-1.14.0.tar.gz.sha256
sha256sum --check hermes-noosphere-memory-1.14.0.tar.gz.sha256
tar -xzf hermes-noosphere-memory-1.14.0.tar.gz
cd hermes-noosphere-memory-1.14.0
./install-hermes.sh
```
...
```bash
git clone --branch v-hermes-1.14.0 --depth 1 https://github.com/SweetSophia/noosphere.git
