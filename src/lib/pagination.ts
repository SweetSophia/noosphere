...
export function parsePagination(
  ...
  const page = Number.isNaN(rawPage) ? (defaults.page ?? 1) : Math.max(1, rawPage);
  const limit = Number.isNaN(rawLimit) ? (defaults.limit ?? 20) : Math.min(Math.max(1, rawLimit), maxLimit);
