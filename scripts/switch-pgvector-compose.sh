...
# base64, and executed as the non-superuser production migration owner. This
# keeps hostile catalog identifiers structured and prevents the bootstrap
# superuser from becoming an identifier-to-COPY-TO-PROGRAM privilege bridge.
_digest_psql() {
...
normalized_dump() {
  ...
  if [[ "$section" == "--schema-only" ]]; then
    if ! docker exec "$container" pg_dump -U noosphere_migrator -d noosphere "$section" --no-owner --no-privileges --inserts 2>/dev/null |
