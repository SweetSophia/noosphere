...
test_authorization_postprocessing_failure_closes_writer_durably() (
  ...
  jq -e '
    .phase == "incident" and
    (.incidentClass | type == "string" and startswith("closure-")) and
...
test_root_owned_writable_compose_parent_is_rejected() (
  local fixture_dir simulated_root_parent safe_dir source target
  fixture_dir=$(mktemp -d)
  trap 'rm -rf -- "$fixture_dir"' EXIT
  simulated_root_parent="$fixture_dir/root-owned-writable"
  safe_dir="$fixture_dir/safe"
  install -d -m 777 "$simulated_root_parent"
  install -d -m 700 "$safe_dir"
  source="$simulated_root_parent/candidate.yml"
  target="$safe_dir/live-compose.yml"
  printf 'candidate-model\n' > "$source"
  printf 'source-model\n' > "$target"
  chmod 0600 "$source" "$target"
  ...
  fsync_path() { :; }
  stat() {
    if [[ $1 == -c && $2 == %u && $3 == "$simulated_root_parent" ]]; then
      printf '0\n'
      return 0
    fi
    command stat "$@"
  ...
  if publish_compose_atomic "$source" "$target"; then
    echo 'publication accepted a root-owned group/world-writable Compose parent' >&2
    exit 1
  fi
  [[ $(<"$target") == source-model ]] || {
    echo 'unsafe root-owned parent rejection changed live Compose' >&2
    exit 1
  }
)
