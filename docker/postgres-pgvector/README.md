...
There is intentionally no broad repository-tag trigger, no mutable version tag, and no `latest` tag. Push publication runs queue rather than cancel one another, and immutable release-tag replacement fails closed. Only push jobs that must write untagged platform digests or promote their verified index receive `packages: write`; pull-request jobs remain read-only.
